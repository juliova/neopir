...
<body>
    ...
    {   
             ...
             if($result["Estado"]='FORMALIZADO')
             {
               echo ("<script LANGUAGE='JavaScript'>
                    window.alert('La prueba ha sido formalizada con exito');
             ...
             if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                  if(strcmp ($row["estadoestudiante.Estado"] ,"SIN REVISAR") == 0){ 
                       echo "<tr class='norevisado' onclick='examenesxestudiante.php?idfecha=".$fecha."&idestudiante=".$row["IDEstudiante"]."><td>" . $row["IDEstudiante"]."</td><td>" . $row["Fechar"]."</td> <td>".$row["estadoestudiante.Estado"]."</td></tr>";
                  }else{
                    echo "<tr class='revisado' onclick='examenesxestudiante.php?idfecha=".$fecha."&idestudiante=".$row["IDEstudiante"]."><td>" . $row["IDEstudiante"]."</td><td>" . $row["Fechar"]."</td> <td>".$row["estadoestudiante.Estado"]."</td></tr>";
                  }  
              } 
            echo "</table> 
  ...
  ?>  
    ...
    <div class="flexCentro">
        <button>Formalizar Revisión</button>
      </div>
    </div>
  </div>
</body>
